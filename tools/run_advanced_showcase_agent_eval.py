"""Run advanced showcase images through debug -> LLM agent -> repair -> eval.

This script is the advanced-showcase counterpart of the full14 harness. It is
meant for the five generated 301-305 cases and keeps API keys out of source
code by reading them from an environment variable.
"""
...
AUTO_APPROVED_BY = "auto_advanced_showcase"
AUTO_APPROVAL_NOTES = "Automatically accepted apply-able repair candidate during advanced showcase run."

RUN_NAME_PREFIX = "advanced_showcase"
RUN_TAG = ""
# ======================================================================
...
ADVANCED_DIR = PROJECT_ROOT / "data" / "generated" / "handdrawn_advanced_showcase"

CASE_SPECS = [
    ("301", ADVANCED_DIR / "301_three_stage_rc_ladder.png"),
    ("302", ADVANCED_DIR / "302_rectangular_bridge_network.png"),
    ("303", ADVANCED_DIR / "303_mixed_parallel_filter_bank.png"),
    ("304", ADVANCED_DIR / "304_dual_loop_multi_shunt.png"),
    ("305", ADVANCED_DIR / "305_dense_supported_grid.png"),
]
...
def _validate_llm_config(env: dict[str, str]) -> None:
    ...
    if not env.get(API_KEY_ENV):
        raise ValueError(f"No API key found. Set ${API_KEY_ENV} before running this script.")
...
def _ensure_advanced_images(python_exe: str, env: dict[str, str], dry_run: bool) -> dict[str, Any] | None:
    missing = [str(path) for _, path in CASE_SPECS if not path.exists()]
    if not missing:
        return None
    command = [
        python_exe,
        "-B",
        "tools/generate_advanced_showcase_circuits.py",
        "--output-dir",
        "data/generated/handdrawn_advanced_showcase",
    ]
    result = _run_command(command, env, dry_run=dry_run)
    _require_success("generate_advanced_showcase_circuits", result)
    return {
...
def run_case(
    ...
    if auto_apply and candidate:
        ...
        elif plan_candidate_ids:
            reason = "No selected apply-able repair candidate."
        else:
            reason = "Advisor prepared no repair plan."
        case_report["auto_apply"]["reason"] = reason
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        f"# Advanced Showcase Agent Run: {report.get('run_id')}",
        "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run advanced showcase LLM agent eval pipeline.")
    parser.add_argument(
        "--python-exe",
        default=sys.executable,
        help="Python executable for subprocesses. Defaults to the current interpreter.",
    )
    parser.add_argument(
        "--output-root",
        default=None,
    ...
    parser.add_argument("--dry-run", action="store_true", help="Write/report commands without executing them.")
    parser.add_argument("--continue-on-error", action="store_true", help="Continue if one case fails.")
    parser.add_argument("--skip-summary", action="store_true", help="Skip final multi-case eval summary.")
    parser.add_argument("--no-auto-apply", action="store_true", help="Disable automatic merge_nodes apply.")
    return parser
...
def main() -> None:
    ...
    setup_result = _ensure_advanced_images(args.python_exe, env, args.dry_run)
    ...
    report = {
        "schema_version": "advanced-showcase-agent-eval-v1",
        "run_id": run_id,
        ...
        "auto_approve_applyable_repairs": not args.no_auto_apply,
        ...
        "outputs": {
            ...
            "json": str(output_root / "advanced_showcase_agent_report.json"),
            "markdown": str(output_root / "advanced_showcase_agent_report.md"),
            "summary_dir": str(output_root / "agent_eval_summary") if summary_result else None,
        },
    }

    json_path = output_root / "advanced_showcase_agent_report.json"
    md_path = output_root / "advanced_showcase_agent_report.md"
